...
#define BITMAP_SIZE 2048  // You can also try 1024 or 4096 if shared memory allows
...
__device__ void processTwoHop(
    ...
    for (unsigned int j = 0; j < len; ++j) {
        ...
        int idx = w - c_nodeoffset;
        if (idx >= 0 && idx < BITMAP_SIZE) {
            if (sign_sum == 0)
                atomicAdd(&bitmap0[idx], 1);
            else if (sign_sum == 1)
                atomicAdd(&bitmap1[idx], 1);
            else if (sign_sum == 2)
                atomicAdd(&bitmap2[idx], 1);
        }
    }
}
...
__global__ void BFCbyHashHybrid(unsigned int totalNodeNum, unsigned int nodenum_select, int nonZeroSize) {
    ...
    extern __shared__ unsigned int shmem[];
    unsigned int* sh_bitmap_0 = shmem;
    unsigned int* sh_bitmap_1 = shmem + BITMAP_SIZE;
    unsigned int* sh_bitmap_2 = shmem + 2 * BITMAP_SIZE;

    // Initialize shared bitmaps
    for (int i = tid; i < BITMAP_SIZE; i += blockDim.x) {
        sh_bitmap_0[i] = 0;
        sh_bitmap_1[i] = 0;
        sh_bitmap_2[i] = 0;
    }
    ...
    for (int i = bid; i < nonZeroSize; i += gridDim.x) {
        ...
        unsigned int twoHopSize = 0;
        for (int vi = tid; vi < curNodeNbrLength; vi += blockDim.x) {
            int v = curNodeNbr[vi];
            twoHopSize += c_offset[v + 1] - c_offset[v];
        }

        __shared__ unsigned int sharedTwoHopSize;
        if (tid == 0) sharedTwoHopSize = 0;
        __syncthreads();
        atomicAdd(&sharedTwoHopSize, twoHopSize);
        __syncthreads();
        twoHopSize = sharedTwoHopSize;

        if (curNodeNbrLength <= 32 && twoHopSize <= 32) {
            if (tid == 0) {
                for (int vi = 0; vi < curNodeNbrLength; ++vi) {
                    int v = curNodeNbr[vi];
                    int sign_uv = getSign(u, v);
                    processTwoHop(u, v, sign_uv, sh_bitmap_0, sh_bitmap_1, sh_bitmap_2);
                }
            }
        } else if (curNodeNbrLength <= 32 && twoHopSize <= 1024) {
            if (threadIdx.x < 32) {
                for (int vi = lane_id; vi < curNodeNbrLength; vi += 32) {
                    int v = curNodeNbr[vi];
                    int sign_uv = getSign(u, v);
    ...
    // Final aggregation
    unsigned long long sum = 0;
    for (int j = tid; j < BITMAP_SIZE; j += blockDim.x) {
        unsigned int two_zeros = sh_bitmap_0[j];
        unsigned int one_zero  = sh_bitmap_1[j];
        unsigned int two_ones  = sh_bitmap_2[j];
...
ll my_exact_hashmap_bfc(edge *d_Edges, int *d_adjLen, int *d_edgeOffset, int *d_edgeRow, int *d_nonZeroRow) {
    ...
    cudaMalloc(&t_sum, sizeof(unsigned long long));
    ...
    // LAUNCH WITH SHARED MEMORY: 3 * nodenum_select * sizeof(unsigned int)
   size_t sharedMemSize = 3 * BITMAP_SIZE * sizeof(unsigned int);
BFCbyHashHybrid<<<blockNum, blockSize, sharedMemSize>>>(nodeNum, nodeNum, nonZeroSize);

if (nodeOffset + BITMAP_SIZE < nodeNum) {
    printf("⚠️ Warning: Some node indices may be ignored! Increase BITMAP_SIZE or use global memory version.\n");
}
